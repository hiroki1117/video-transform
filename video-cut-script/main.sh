...
# 動画のトリミング
ffmpeg -ss "${SS}" -i ${DL_FILENAME} -t "${DURATION}" "${TRIM_FILENAME}"

# フェードアウト処理
if [ $FADEOUT = "true" ]; then
    FADE_FILENAME="fadetemp.mp4"
    ffmpeg -i "${TRIM_FILENAME}" -vf "fade=d=1.5,reverse,fade=d=1.5,reverse" -af "afade=t=in:st=0:d=2.5,areverse,afade=t=in:st=0:d=2.5,areverse" "${FADE_FILENAME}"
fi
